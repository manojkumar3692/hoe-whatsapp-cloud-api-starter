...
import { gstSplit } from "./accounting";
...
export async function renderInvoice(order: any): Promise<Buffer> {
  ...
  const { taxable: taxableValuePaise, gst: totalGstPaise, cgst: cgstPaise,
    sgst: sgstPaise, igst: igstPaise, intraState, knownState } = gstSplit(grandTotalPaise, order.customer_state);
  ...
  doc.text(`Place of Supply: ${order.customer_state || "-"} (${!knownState ? "Needs review" : intraState ? "Intra-state" : "Inter-state"})`);
  doc.moveDown(1);
  ...
  // ------------------------------------------------------------------
  const totalsX = 310;
  const totalAmountX = 455;
  doc.font("Helvetica").fontSize(10).fillColor("#333");

  doc.text("Taxable Value", totalsX, y);
  doc.text(formatINR(taxableValuePaise), totalAmountX, y, { width: 90, align: "right" });
  y += 16;

  if (!knownState) {
    doc.text("GST @ 18% (review split)", totalsX, y);
    doc.text(formatINR(totalGstPaise), totalAmountX, y, { width: 90, align: "right" });
    y += 16;
  } else if (intraState) {
    doc.text("CGST @ 9%", totalsX, y);
    doc.text(formatINR(cgstPaise), totalAmountX, y, { width: 90, align: "right" });
    y += 16;

    doc.text("SGST @ 9%", totalsX, y);
    doc.text(formatINR(sgstPaise), totalAmountX, y, { width: 90, align: "right" });
    y += 16;
  } else {
    doc.text("IGST @ 18%", totalsX, y);
    doc.text(formatINR(igstPaise), totalAmountX, y, { width: 90, align: "right" });
    y += 16;
  }
  ...
  doc.text("Grand Total (incl. GST)", totalsX, y, { width: 140 });
  doc.text(formatINR(grandTotalPaise), totalAmountX, y, { width: 90, align: "right" });
  y += 28;
  ...
  if (order.payment_type === "partial_cod") {
    doc.text(
      `Payment: ${formatINR(order.token_amount_in_paise)} paid online, ` +
        `${formatINR(order.balance_due_in_paise)} ${order.cod_balance_status === "collected" ? "recorded collected on delivery" : "payable by cash/UPI on delivery"}. ` +
