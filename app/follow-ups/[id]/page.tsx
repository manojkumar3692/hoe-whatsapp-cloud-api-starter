...
export default async function FollowupDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string }> }) {
  ...
  return <main style={{ padding: 24, background: "#faf8f4", minHeight: "100vh", color: "#1c1712" }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.1fr) minmax(310px, .9fr)", gap: 16, alignItems: "start" }}>
      ...
      <div style={{ display: "grid", gap: 16 }}>
        <section style={card}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}><a href={`tel:+${phone}`} style={{ ...button, background: "#166534", textDecoration: "none" }}>☎ Call {order?.customer_phone || customer?.phone}</a><Link href={`/inbox/${phone}`} style={{ ...button, background: "#2563eb", textDecoration: "none" }}>WhatsApp</Link></div>
